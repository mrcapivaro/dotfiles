...
local function get_aligned_layout(panel_position)
    return is_horizontal_position(panel_position)
...
local function get_fixed_layout(panel_position)
    return is_horizontal_position(panel_position)
            and wibox.layout.fixed.horizontal
...
local function create_left_layout(screen_index, panel)
    ...
    local tag_list = awful.widget.taglist({
        ...
        widget_template = {
            {
                {
                    {
                        {
                            forced_width = is_horizontal_position(
                                panel.position
                            )
                                    and panel.thickness - dpi(6)
                                or 0,
                            forced_height = is_horizontal_position(
                                panel.position
                            )
                                    and 0
                                or panel.thickness - dpi(6),
                    ...
                    top = dpi(
                        is_horizontal_position(panel.position) and 2 or 8
                    ),
                    right = dpi(
                        is_horizontal_position(panel.position) and 8 or 2
                    ),
                    left = dpi(
                        is_horizontal_position(panel.position) and 8 or 2
                    ),
            ...
            create_callback = function(self, _, index, _)
                ...
                self:get_children_by_id("tasklist")[1]:add(
                    create_tasklist(
                        screen_index,
                        panel.position,
                        tag,
    ...
    local launcher_margin_container =
        wibox.container.margin(panel.launcher.icon)
    launcher_margin_container.margins = dpi(2)

    local left_layout_widgets = create_fixed_layout(
        panel.position,
        { launcher_margin_container, taglist_margin_container }
    )
...
local function create_right_layout(screen_index, panel)
    ...
    for _, widget_or_widget_group in ipairs(panel.widgets) do
        ...
        local widgets = is_widget_group and widget_or_widget_group
            or { widget_or_widget_group }
        local containers = {}

        for i, widget in ipairs(widgets) do
            if widget.icon or widget.value then
                if widget.icon then
                    local widget_icon_margin_container =
                        wibox.container.margin(widget.icon)
                    widget_icon_margin_container.margins = dpi(1)
                    table.insert(containers, widget_icon_margin_container)
                end
                if widget.value then
                    local widget_value_rotate_container =
                        wibox.container.rotate(
                            widget.value,
                            get_direction_by_position(panel.position)
                        )
        ...
        if #containers == 1 then
            ...
            local widget_fixed_container =
                create_fixed_layout(panel.position, containers)

            local widgets_margin_container =
                wibox.container.margin(widget_fixed_container)
            ...
            widget_background_container.shape_border_color =
                beautiful.multi_widget_border_color

            top_level_container =
                wibox.container.margin(widget_background_container)
